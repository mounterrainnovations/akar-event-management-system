  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        ...
        // Center the QR code
        // A4 width is 595. QR width is 150. Center x = (595 - 150) / 2 = 222.5
        doc.image(qrCodeDataUrl, 222.5, footerY - 50, {
          width: 150,
          height: 150,
        });
        doc
          ...
          .font("Helvetica")
          .fillColor("#000000")
          .text("Scan for entry", 50, footerY + 110, {
      ...
      // First Page Footer Branding
      doc
        .fontSize(8)
        .font("Helvetica-Oblique")
        .fillColor("#AAAAAA")
        .text("Generated by Akar Event Management System", 50, 780, {
          align: "center",
        });

      // --- 5. Second Page: Terms & Conditions ---
      doc.addPage();

      // --- 5a. Header (Copy of First Page Header) ---
      try {
        const possiblePaths = [
          path.resolve(process.cwd(), "public", "logo.png"),
          path.resolve(process.cwd(), "backend", "public", "logo.png"),
      ...
      const termsWidth = 495; // Full width (595 - 50 - 50)

      if (details.eventTerms) {
        doc
          .fontSize(12)
          .font("Helvetica-Bold")
          .text("EVENT TERMS & CONDITIONS", 50, termsY);
        ...
        doc
          .fontSize(10)
          .font("Helvetica")
          .text(details.eventTerms, 50, termsY, {
            width: termsWidth,
            lineGap: 4,
            align: "justify",
          });

        // Estimate height of text to push next section down
        const textHeight = doc.heightOfString(details.eventTerms, {
          width: termsWidth,
          lineGap: 4,
        });
        termsY += textHeight + 40;
      }

      doc
        .fontSize(12)
        .font("Helvetica-Bold")
        .text("TICKET TERMS & CONDITIONS", 50, termsY);
      ...
      doc.fontSize(10).font("Helvetica").text(standardTerms, 50, termsY, {
        width: termsWidth,
        lineGap: 5,
        align: "left",
      });

      // Second Page Footer Branding
      doc
        .fontSize(8)
        .font("Helvetica-Oblique")
        .fillColor("#AAAAAA")
