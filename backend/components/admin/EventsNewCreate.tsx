...
export function EventsNewCreate({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof EventFormData | string, string>>
  >({});
  ...
  const validateStep1 = () => {
    ...
    if (!formData.name.trim()) {
      ...
      // Validate event name - only allow alphanumeric, spaces, and common punctuation
    ...
    if (!formData.baseEventBanner)
      newErrors.baseEventBanner = "Event banner is required";
    ...
    const validTerms = formData.termsAndConditions.filter(
      (t) => t.trim().length > 0,
    );
    if (validTerms.length < 3) {
      newErrors.termsAndConditions =
        "At least 3 terms and conditions are required";
    }
  ...
  const validateStep3 = () => {
    ...
    formData.coupons.forEach((coupon, index) => {
      if (!coupon.code.trim())
        newErrors[`coupon_${index}_code`] = "Code is required";
      if (coupon.discountValue <= 0)
        newErrors[`coupon_${index}_value`] = "Value must be greater than 0";
    });
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
    ...
    formData.formFields.forEach((field, index) => {
      ...
      if (field.fieldType === "dropdown") {
        field.options.forEach((opt, optIndex) => {
          if (!opt.value.trim())
            newErrors[`field_${index}_option_${optIndex}`] =
  ...
  const validateStep5 = () => {
    ...
    formData.tickets.forEach((ticket, index) => {
      if (!ticket.name.trim())
        newErrors[`ticket_${index}_name`] = "Name is required";
      if (!ticket.brief.trim())
        newErrors[`ticket_${index}_brief`] = "Brief is required";
      if (ticket.price < 0)
        newErrors[`ticket_${index}_price`] = "Price cannot be negative";
      if (ticket.quantity < 1)
        newErrors[`ticket_${index}_quantity`] = "Quantity must be at least 1";
      if (ticket.maxQuantityPerPerson < 1)
        newErrors[`ticket_${index}_maxQuantityPerPerson`] =
  ...
  const validateStep6 = () => {
    ...
    formData.bundleOffers.forEach((offer, index) => {
      if (!offer.name.trim())
        newErrors[`offer_${index}_name`] = "Name is required";
      if (offer.buyQuantity < 1)
        newErrors[`offer_${index}_buyQuantity`] = "Buy qty must be at least 1";
      if (offer.getQuantity < 1)
        newErrors[`offer_${index}_getQuantity`] = "Get qty must be at least 1";
    });
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
  ...
  const updateNestedField = (
    ...
    field: string,
    value: any,
    listName: "coupons" | "formFields" | "tickets",
  ) => {
    setFormData((prev) => ({
      ...prev,
    ...
    const errorKey =
      listName === "coupons"
        ? `coupon_${index}_${field}`
        : listName === "tickets"
          ? `ticket_${index}_${field}`
  ...
  const updateFormFieldOptionValue = (
    index: number,
    optionIndex: number,
    value: string,
  ) => {
  ...
  const updateFormFieldOptionTriggers = (
    ...
    setFormData((prev) => {
      ...
      if (isAdded) {
        if (!currentTriggers.includes(triggerField))
          currentTriggers = [...currentTriggers, triggerField];
      } else {
  ...
  const addFormFieldOption = (index: number) => {
    setFormData((prev) => {
      ...
      newFormFields[index] = {
        ...newFormFields[index],
        options: [
          ...newFormFields[index].options,
          { value: "", label: "", triggers: [] },
        ],
      };
      return { ...prev, formFields: newFormFields };
    });
  ...
  const removeFormFieldOption = (index: number, optionIndex: number) => {
    setFormData((prev) => {
      const newFormFields = [...prev.formFields];
      const newOptions = newFormFields[index].options.filter(
        (_, i) => i !== optionIndex,
      );
      newFormFields[index] = { ...newFormFields[index], options: newOptions };
      return { ...prev, formFields: newFormFields };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {getStepDetails(currentStep).label}
              </p>
            </div>
          </div>
        </div>
              ...
              {STEPS.map((step, idx) => {
                ...
                return (
                  <div key={step} className="flex items-center gap-2">
                    <div
                      className={`flex size-8 items-center justify-center rounded-full border-2 text-sm font-bold ${colorClass}`}
                    >
                      {isCompleted ? <CheckCircle weight="bold" /> : stepNum}
                    </div>
                    {idx < STEPS.length - 1 && (
                      <div className="hidden sm:block h-0.5 w-8 bg-border"></div>
                    )}
                  </div>
                );
              })}
            ...
            {currentStep === "general" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-2">
                  ...
                  <div
                    className={`relative flex h-48 w-full flex-col items-center justify-center rounded-lg border-2 border-dashed transition-colors ${errors.baseEventBanner ? "border-red-500/50 bg-red-500/5" : "border-border/60 bg-muted/5 hover:bg-muted/10"}`}
                  >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <DateTimePicker
                      ...
                      setDate={(date) =>
                        updateField("registrationStart", date.toISOString())
                      }
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <DateTimePicker
                      ...
                      setDate={(date) =>
                        updateField("registrationEnd", date.toISOString())
                      }
                    />
                  ...
                  <div className="space-y-2">
                    {formData.termsAndConditions.map((term, index) => (
                      <div key={index} className="flex gap-2">
                        <span className="py-2 text-xs font-mono text-muted-foreground">
                          {index + 1}.
                        </span>
                        <input
                          type="text"
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-2 pt-2 border-t">
                  ...
                  <p className="text-[10px] text-muted-foreground italic leading-relaxed">
                    Provide a link to the venue on Google Maps or other map
                    services.
                    <br />
                    <span className="text-primary/70 font-medium">
                      Tip: Go to Google Maps, select your location, click
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="space-y-4">
                                {field.options.map((option, optIndex) => (
                                  <div
                                    key={optIndex}
                                    className="p-3 bg-muted/30 rounded-md border border-border space-y-3"
                                  >
                                    ...
                                    <div className="pl-6">
                                      <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1.5">
                                        Triggers (Show fields when selected)
                                      </p>
                                          ...
                                          (targetField, tfIndex) => {
                                            if (
                                              ...
                                              return (
                                                <label
                                                  key={tfIndex}
                                                  className={`flex items-center gap-1.5 px-2 py-1 rounded border text-xs cursor-pointer ${isTriggered ? "bg-primary/10 border-primary text-primary" : "bg-background border-input text-muted-foreground"}`}
                                                >
                                                  <input
                                                    type="checkbox"
                                                    checked={
                      ...
                      <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            min="1"
                            value={
                              isNaN(ticket.quantity) ? "" : ticket.quantity
                            }
                            onChange={(e) =>
                              updateNestedField(
                                index,
                        ...
                        <div className="space-y-1.5">
                          ...
                          />
                          {errors[`ticket_${index}_maxQuantityPerPerson`] && (
                            <p className="text-xs text-red-500">
                              {errors[`ticket_${index}_maxQuantityPerPerson`]}
                            </p>
                          )}
                              ...
                              return (
                                <div key={fIdx} className="text-xs">
                                  <div className="flex items-center gap-2 mb-1.5">
                                    <span
                                      className={`font-medium ${hasActiveRules ? "text-primary" : "text-foreground"}`}
                                    >
                                      {field.label}
                                    </span>
                                    {hasActiveRules && (
                                      <span className="text-[9px] text-primary bg-primary/5 px-1 rounded">
                                        Active
                                      </span>
                                    )}
                                  </div>
                ...
                <div className="space-y-6">
                  {formData.bundleOffers.length === 0 ? (
                    ...
                    formData.bundleOffers.map((offer, index) => (
                      <div
                        key={index}
                        className="rounded-lg border border-border p-5 space-y-4 relative group bg-card/50 shadow-sm transition-all hover:shadow-md hover:border-border/80"
                      >
                          ...
                          <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-1.5">
                              ...
                              >
                                ...
                                <SelectContent>
                                  <SelectItem value="same_tier">
                                    Same Tier Only
                                  </SelectItem>
                                  <SelectItem value="cross_tier">
                                    Across Tiers (Cheapest Free)
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5">
                                ...
                                />
                                {errors[`offer_${index}_buyQuantity`] && (
                                  <p className="text-xs text-red-500">
                                    {errors[`offer_${index}_buyQuantity`]}
                                  </p>
                                )}
                              ...
                              <div className="space-y-1.5">
                                ...
                                />
                                {errors[`offer_${index}_getQuantity`] && (
                                  <p className="text-xs text-red-500">
                                    {errors[`offer_${index}_getQuantity`]}
                                  </p>
                                )}
                          ...
                          <div className="space-y-2 pt-2">
                            <label className="text-xs font-medium flex items-center justify-between">
                              ...
                              <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">
                                {offer.applicableTicketIds.length === 0
                                  ? "All Tiers"
                                  : `${offer.applicableTicketIds.length} Selected`}
                              </span>
                            </label>
                            ...
                            <p className="text-[10px] text-muted-foreground">
                              If no tiers are selected, the offer applies to ALL
                              tiers.
                            </p>
                          </div>
                        </div>
